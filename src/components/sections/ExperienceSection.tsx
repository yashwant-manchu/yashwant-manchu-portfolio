...
const experiences = [
  ...
  },
  {
    ...
    current: false,
    achievements: [
      'Developed the company marketing website using HTML5, CSS3 & JavaScript, deployed to production via GitHub Pages',
  ...
  return (
      <section id="experience" className="section-padding" style={{ background: 'var(--bg-primary)' }}>
        <div className="container mx-auto px-4">
          <motion.div ref={ref} variants={container} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="max-w-5xl mx-auto">
            ...
            <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold mb-4 uppercase tracking-widest"
              ...
              <h2 className="text-4xl md:text-5xl font-extrabold gradient-text-blue mb-4">Work Experience</h2>
              <div className="w-16 h-1 rounded-full mx-auto" style={{ background: 'linear-gradient(90deg, var(--accent), var(--amber))' }} />
            </motion.div>

            {/* Timeline */}
            <div className="relative space-y-8">
              {/* Vertical line */}
              <div className="absolute left-6 top-3 bottom-3 w-px hidden md:block"
                   style={{ background: 'linear-gradient(180deg, var(--accent), rgba(245,158,11,0.3), transparent)' }} />

              {experiences.map((exp, i) => (
                  <motion.div key={i} variants={item} className="relative pl-0 md:pl-16">
                    {/* Timeline dot */}
                    <div className="absolute left-4 top-6 hidden md:flex items-center justify-center w-5 h-5 rounded-full border-2"
                         style={{
                    ...
                    <motion.div className="glass-card rounded-2xl p-6 hover-lift" whileHover={{ scale: 1.01 }}>
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                        ...
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold"
                              style={{
                                background: exp.current ? 'var(--accent-light)' : exp.type === 'Internship' ? 'var(--amber-light)' : 'var(--bg-tertiary)',
                                color: exp.current ? 'var(--accent-dark)' : exp.type === 'Internship' ? 'var(--amber)' : 'var(--text-muted)',
                              }}>
                          {exp.current ? '● Current' : exp.type}
                        </span>
                          </div>
                          <h3 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{exp.position}</h3>
                        ...
                        <div className="text-right text-xs space-y-1 flex-shrink-0" style={{ color: 'var(--text-muted)' }}>
                      ...
                      <ul className="space-y-2 mb-5">
                        {exp.achievements.map((a, j) => (
                            <motion.li key={j} className="flex items-start gap-2 text-sm" style={{ color: 'var(--text-secondary)' }}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.techStack.map((t) => (
                            <span key={t} className="px-2.5 py-0.5 rounded-full text-xs font-medium"
                                  style={{ background: 'var(--bg-tertiary)', color: 'var(--text-muted)', border: '1px solid var(--border-color)', fontFamily: "'DM Mono', monospace" }}>
                        {t}
                      </span>
                        ))}
                      </div>
              ...
              <div className="grid sm:grid-cols-3 gap-5">
                {certifications.map((c, i) => (
                    <motion.div key={i}
                      ...
                      <Award className="w-7 h-7 mx-auto mb-3" style={{ color: 'var(--amber)' }} />
