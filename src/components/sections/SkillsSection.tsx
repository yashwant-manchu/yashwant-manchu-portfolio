...
const categories = [
  {
    ...
    skills: [
      ...
      { name: 'Material-UI / Gluestack', level: 80 },
    ],
  },
  {
    id: 'state',
  ...
  },
  {
    ...
    skills: [
      ...
      { name: 'VS Code / WebStorm', level: 94 },
      { name: 'Vite / Webpack', level: 78 },
      { name: 'Bitbucket / Jira', level: 76 },
      { name: 'Vercel / GitHub Pages', level: 82 },
      { name: 'Expo CLI', level: 86 },
    ],
  },
  {
    id: 'testing',
    ...
    skills: [
      ...
      { name: 'Expo (Managed & Bare)', level: 86 },
      { name: 'Google Play Store Deploy', level: 85 },
      { name: 'Apple App Store Deploy', level: 82 },
      { name: 'NativeWind / TamagUI', level: 78 },
    ],
  },
];
        ...
        <div className="container mx-auto px-4">
          <motion.div ref={ref} variants={container} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="max-w-5xl mx-auto">
            ...
            <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold mb-4 uppercase tracking-widest"
              ...
              <h2 className="text-4xl md:text-5xl font-extrabold gradient-text-blue mb-4">Skills &amp; Tech Stack</h2>
            ...
            >
              {activeCat.skills.map((skill, i) => (
                  ...
                  <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)', fontFamily: "'DM Mono', monospace" }}>
                    {skill.name}
                  </span>
                      <span className="text-xs font-medium" style={{ color: 'var(--text-muted)' }}>{skill.level}%</span>
                    </div>
            ...
            <motion.div variants={item} className="mt-14 grid grid-cols-2 sm:grid-cols-4 gap-5">
              {[
                { value: '3+', label: 'Years building', color: 'var(--accent)' },
                { value: '20+', label: 'Technologies', color: 'var(--amber)' },
                { value: '10+', label: 'Projects shipped', color: '#06b6d4' },
                { value: '85%+', label: 'Test coverage', color: '#a78bfa' },
              ].map((s) => (
